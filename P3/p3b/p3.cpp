...
bool cStringList::deleteAt(int index, string &text) {
    ...
    if ((index >= 0) && (index < listCount)) {
        int i = first;
        //walk through the list strarting from first
        while (i <= index) {
            //use incVal to ensure proper wrapping
            //this also allows us to use the index value 
            incVal(i);
        }
...
bool cStringList::readAt(int index, string &text) {
    ...
    if ((index >= 0) && (index < listCount)) {
        ...
        while (i < index) {
            i++;
            //use incVal to allow proper wrapping 
            //this ensures that index is correct
            incVal(read);
        }
        text = a[read];
        rc = true;
